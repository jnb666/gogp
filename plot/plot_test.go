...
func TestPlot(t *testing.T) {
    ...
    checkError(t, err)
    t.Log(problem.GetHistory("fitMax"))
    err = p.AddLine(problem, "fitMax")
    checkError(t, err)
    t.Log(problem.GetHistory("fitAvg"))
    t.Log(problem.GetHistory("fitStd"))
    err = p.AddLineErrors(problem, "fitAvg", "fitStd")
    checkError(t, err)
