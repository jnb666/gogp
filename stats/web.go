...
func (l *Logger) InitChan() (chan bool) {
    l.start = make(chan bool)
    l.step = make(chan bool)
    return l.start
}
...
// handler to listen on channel
func handleChannel(pattern string, ch chan bool) {
    if ch != nil {
        http.HandleFunc(pattern,
            func (w http.ResponseWriter, r *http.Request) { sendJSON(w, <-ch) })
    }
}
