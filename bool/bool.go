...
var (
    True  = V(true)
    False = V(false)
    And = Op("and", func(a []V)V { return a[0] && a[1] })
    Or = Op("or", func(a []V)V { return a[0] || a[1] })
    Xor = Op("xor", func(a []V)V { return (a[0] || a[1]) && !(a[0] && a[1]) })
    Not = Func("not", 1, func(a []V)V { return !a[0] })
)
...
// Func constructor returns a boolean function with given arity which implements 
...
func Op(name string, fun func([]V) V) expr.Opcode {
    return boolFunc{ expr.Operator(name), fun }
}
